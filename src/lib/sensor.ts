...
export function calcSensor(
    refs: { [k: string]: HTMLElement | HTMLElement[] },
    scale: [number, number]
) {
    ...
    if (originCSSMatrix == "none") {
        ...
        const originMatrix = originCSSMatrix
            .match(/-?\d+\.?\d*/g)
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const { width: sensorWidth, height: sensorHeight } =
            sensorEl.getBoundingClientRect();
        ...
        if (sensorCSSMatrix == "none") {
            ...
            const sensorMatrix = sensorCSSMatrix
                .match(/-?\d+\.?\d*/g)
                ?.map((val) => parseFloat(val))!;
        ...
        const sensorRotatedX =
            Math.cos(-originR) * (sensorX - originX) -
            Math.sin(-originR) * (sensorY - originY);
        const sensorRotatedY =
            Math.sin(-originR) * (sensorX - originX) +
            Math.cos(-originR) * (sensorY - originY);

        const sensorNormalizedX =
            Math.floor(((sensorRotatedX / canvasBBox.width) * scale[0]) / 10) *
            10;
        const sensorNormalizedY =
            Math.floor(((sensorRotatedY / canvasBBox.height) * scale[1]) / 10) *
            10;
        sensorLocations.push(<[number, number]>[
            sensorNormalizedX,
            sensorNormalizedY,
        ]);
    }
